...
- **实时矢量预览** — 转换后即时在右侧渲染矢量形状，支持亮度调节滑块，便于核对效果。
...
- **暂未反向还原变换标签**：输出固定为几何路径，不会把 `\fscx/\fscy/\frz/\pos` 还原成 SVG `transform` 属性。如需精确还原原始位置，请在 Aegisub 中先「Apply Tags to All」，或在转换后手动添加 SVG `transform`。
- **暂未还原颜色**：输出 SVG 默认黑色填充（`fill="#000"`），不解析 `\c&Hbbggrr&` 标签。
- **暂未支持 B-spline**：`s/p/c` 三次 B 样条曲线命令在 v1 中被跳过。
...
- **功能开发** — 颜色还原、变换标签还原、B-spline 支持、bug 修复、性能优化
